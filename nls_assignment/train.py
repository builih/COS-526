...
class LightSphereModel(pl.LightningModule):
    ...
    def solve_sphere_crossings(ray_origins, ray_directions):
        ...
        #----- intersections: torch.Tensor, of shape [N, 3]. Holds unit sphere crossings with specified rays.
        #
        #----- HINTS:
        ...
        a = (ray_directions * ray_directions).sum(dim=1, keepdim=True)
        b = 2.0 * (ray_origins * ray_directions).sum(dim=1, keepdim=True)
        c = (ray_origins * ray_origins).sum(dim=1, keepdim=True) - 1.0
        discriminant = b * b - 4.0 * a * c
        t = (-b + torch.sqrt(torch.clamp(discriminant, min=0.0))) / (2.0 * a)
        intersections = ray_origins + t * ray_directions
    ...
    def inference(self, t, uv, ray_origins, ray_directions, training_phase=1.0):
        ...
        #------------------------------------------------------
        #----- PLEASE FILL IN COMPUTATIONS FOR 
        #----- encoded_offset_position: torch.Tensor, of shape [N, D1], where D1 is some fixed dimension of encoding.
        #----- encoded_offset_angle: torch.Tensor, of shape [N, D2], where D2 is some fixed dimension of encoding.
        #----- encoded_color_position: torch.Tensor, of shape [N, D3], where D3 is some fixed dimension of encoding.
        #----- encoded_color_angle: torch.Tensor, of shape [N, D4], where D4 is some fixed dimension of encoding.
        #
        #----- HINTS:
        #-----   - TCNN configs for the encodings only take in N-dimensional input, where each input ranges in [0, 1].
        #-----   - intersections_sphere: torch.Tensor, of shape [N, 3]. These are points on the surface of a unit sphere.
        #-----   - uv: torch.Tensor, of shape [N, 2]. Both u and v lie within [0, 1].
        #------------------------------------------------------
        
        sphere_normalized = intersections_sphere * 0.5 + 0.5

        if not self.args.no_offset:
            encoded_offset_position = self.encoding_offset_position(sphere_normalized)
        ...
        sphere_offset_normalized = intersections_sphere_offset * 0.5 + 0.5
        encoded_color_position = self.encoding_color_position(sphere_offset_normalized)
    ...
    def forward(self, t, uv, ray_origins, ray_directions, training_phase):
        ...
        if training_phase > 0.2 and not self.args.no_offset:
            encoded_offset_position = self.mask(encoded_offset_position, training_phase, initial=0.2)
            encoded_offset_angle = self.mask(encoded_offset_angle, training_phase, initial=0.5)
        ...
        sphere_offset_normalized = intersections_sphere_offset * 0.5 + 0.5
        encoded_color_position = self.encoding_color_position(sphere_offset_normalized)
        encoded_color_position = self.mask(encoded_color_position, training_phase, initial=0.8)
        
        feat_color = self.network_color_position(encoded_color_position).float()

        if training_phase > 0.25 and not self.args.no_view_color:
            encoded_color_angle = self.mask(encoded_color_angle, training_phase, initial=0.2)
...
class PanoModel(pl.LightningModule):
    ...
    def generate_ray_directions(self, uv, camera_to_world, intrinsics_inv):
        ...
        #------------------------------------------------------
        #----- PLEASE FILL IN COMPUTATIONS FOR 
        #----- x_distorted: torch.Tensor, of shape [N, 1]. Holds distorted x coordinate values.
        #----- y_distorted: torch.Tensor, of shape [N, 1]. Holds distorted y coordinate values.
        #
        #----- HINTS:
        #-----   - x: torch.Tensor, of shape [N, 1].
        #-----   - y: torch.Tensor, of shape [N, 1].
        #-----   - kappa1, ..., kappa5: distotion coefficients k1, k2, k3, p1, p2; as specified in https://docs.opencv.org/4.x/dc/dbb/tutorial_py_calibration.html
        #------------------------------------------------------
        ...
        y_distorted = y * (1 + kappa1 * r2 + kappa2 * r4 + kappa3 * r6) + kappa4 * (r2 + 2 * y**2) + 2 * kappa5 * x * y
