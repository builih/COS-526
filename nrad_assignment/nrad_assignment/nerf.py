...
class NeRF(nn.Module):
    def __init__(self, D=8, W=256, output_ch=4, skips=[4], embedder=PositionalEncoder(10,0), embedder_dirs=None, use_viewdirs=False, input_ch=3, input_ch_views=3):
        ...
        self.pts_linears = nn.ModuleList([nn.Linear(input_ch, W)] + [nn.Linear(W, W) if i not in self.skips else nn.Linear(input_ch + W, W) for i in range(D-1)]) # TODO: Pytorch ModuleList. Holds the MLP architecture from Fig 7 before concatenating with viewing directions
        ...
        if use_viewdirs:
            self.feature_linear = nn.Linear(W, W) # TODO: Pytorch Module. The MLP layer that outputs a feature vector, that will later be concatenated with encoding of viewing directions
            self.alpha_linear = nn.Linear(W, 1) # TODO: Pytorch Module. The MLP layer that outputs volume density, only from 3D coordinates
            self.rgb_linear = nn.Linear(W // 2, 3) # TODO: Pytorch Module. The MLP layer that emits RGB radiance, from 3D coordinates and view direction
        else:
