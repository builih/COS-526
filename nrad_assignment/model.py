...
class Model:
    ...
    def render_video(self, dataset, basedir, expname, step_i):
        ...
        moviebase = os.path.join(basedir, expname, '{}_spiral_{:06d}_'.format(expname, step_i))
        imageio.mimwrite(moviebase + 'rgb.mp4', to8b(rgbs), fps=30, quality=8)
        disps_clean = np.nan_to_num(disps, nan=0.0)
        imageio.mimwrite(moviebase + 'disp.mp4', to8b(disps_clean / np.nanmax(disps_clean)), fps=30, quality=8)
    ...
    def render_only(self, dataset, basedir, expname, render_test=False, render_factor=1.):
        ...
        disps_clean = np.nan_to_num(disps, nan=0.0)
        imageio.mimwrite(os.path.join(testsavedir, 'disp.mp4'), to8b(disps_clean / np.nanmax(disps_clean)), fps=30, quality=8)