...
class Renderer(nn.Module):
    ...
    def ray_marcher(self, raw, z_vals, rays_d, pytest=False):
        ...
        def raw2alpha(raw, dists, act_fn=F.relu):
            ...
            alpha = 1. - torch.exp(-act_fn(raw) * dists) # TODO: [N_rays, N_samples]. Actual alpha values computed from volume density and sampled distance
        ...
        if self.training and self.raw_noise_std > 0.:
            noise = torch.randn(raw[..., 3].shape, device=device) * self.raw_noise_std
        ...
        weights = alpha * torch.cumprod(torch.cat([torch.ones((alpha.shape[0], 1), device=device), 1.-alpha + 1e-10], -1), -1)[:, :-1] # TODO: [num_rays, num_samples]. Weights assigned to each sampled color
        rgb_map = torch.sum(weights[..., None] * rgb, -2) # TODO: [num_rays, 3]. Estimated RGB color of a ray

        depth_map = torch.sum(weights * z_vals, -1) # TODO: [num_rays]. Estimated distance to object
        disp_map = 1./torch.max(1e-10 * torch.ones_like(depth_map), depth_map / torch.sum(weights, -1)) # TODO: [num_rays]. Disparity map. Inverse of depth map.
        acc_map = torch.sum(weights, -1) # TODO: [num_rays]. Sum of weights along each ray
    ...
    def render_rays(self,
        ...
        pts = rays_o[..., None, :] + rays_d[..., None, :] * z_vals[..., :, None] # TODO: [N_rays, N_samples, 3] 3D point position

        # Pass sampled points through an implicit scene representation model
        raw = nerf(pts, viewdirs, self.netchunk)
        rgb_map, disp_map, acc_map, weights, depth_map = self.ray_marcher(raw, z_vals, rays_d, pytest=pytest)

        if self.N_importance > 0:
            rgb_map_0, disp_map_0, acc_map_0 = rgb_map, disp_map, acc_map

            z_vals_mid = .5 * (z_vals[..., 1:] + z_vals[..., :-1])
            z_samples = sample_pdf(z_vals_mid, weights[..., 1:-1], self.N_importance, det=(perturb == 0.), pytest=pytest)
    ...
    def batchify_rays(self, rays_flat, nerf, nerf_fine=None, chunk=1024 * 32, retraw=False):
        """Render rays in smaller minibatches to avoid OOM.
        """
        all_ret = {}
        for i in range(0, rays_flat.shape[0], chunk):
            ret = self.render_rays(rays_flat[i:i + chunk], nerf, nerf_fine=nerf_fine, retraw=retraw)
            for k in ret:
                if k not in all_ret:
                    all_ret[k] = []
    ...
    def forward(self, H, W, K, nerf, nerf_fine=None, chunk=1024*32, rays=None, c2w=None,
                near=0., far=1., c2w_staticcam=None, retraw=False):
        """Render rays
        Args:
    ...
    def render_path(self, render_poses, hwf, K, chunk, nerf, nerf_fine, near, far, gt_imgs=None, savedir=None, render_factor=0,):
        ...
        for i, c2w in enumerate(tqdm(render_poses)):
            rgb, disp, acc, _ = self.forward(H, W, K, nerf, nerf_fine=nerf_fine,
                                             chunk=chunk, c2w=c2w[:3, :4], near=near, far=far)
